...
internal class LogEntry : BindableElement
{
    ...
    internal DateTime TimeDate
    {
        ...
        {
            _timeDate = value;
            if (UI.Instance.ConfigShowTimeStamps.Value)
                _timeDateLabel.text = value.ToString(UI.Instance.ConfigTimeStampFormat.Value);
            else
    ...
    public ILogger LogSource
    {
        ...
        private set
        {
            ...
            _logSource = value;
            _logSourceLabel.text = $"{_logSource.Name}]";
        }
    }
    private ILogger _logSource;
